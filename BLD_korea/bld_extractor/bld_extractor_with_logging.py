...
def main():
	release = "2018_03"
	logging.basicConfig(filename=r"C:\Users\pruszyns\Desktop\python-repo\python\BLD_korea\bld_extractor\{} bld-extractor.log".format(time.strftime("%d-%m-%Y %I-%M-%S")), level=logging.DEBUG, filemode="w", format="%(levelname)s %(asctime)s %(message)s", datefmt="%m/%d/%Y %I:%M:%S ")
